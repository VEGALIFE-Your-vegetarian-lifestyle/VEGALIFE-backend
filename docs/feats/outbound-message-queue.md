...
- [x] FR-001: Register, forgot-password, and resend enqueue an
...
- [x] FR-006: Before each send, `now > expires_at` marks the message
      `EXPIRED` without sending (OTP is already dead — precedence over
      retry scheduling).
- [x] FR-007: Messages older than the 24h maximum age are marked
      `FAILED`; every terminal transition (`COMPLETED`, `FAILED`,
      `EXPIRED`) clears `payload`.
- [x] FR-008: A `PROCESSING` row idle past the 60s visibility timeout
      is reclaimed by the reaper: `PENDING` if `attempts < 4`, else
      `DEFERRED`.
- [x] FR-009: A message that exhausts retries and passes its deadline
      is persisted (`FAILED`/`DEFERRED`) rather than lost — queue
      state survives restarts.
- [x] FR-010: Delivery outcomes are logged as per-event WARN/ERROR
      plus a summary counter (sent, failed, retried) at most every
      60s; payloads and OTPs never appear in logs.
- [x] FR-011: The drainer (and all `@Scheduled` jobs) only run when
      `app.scheduling.enabled` is true.
- [x] FR-012: Terminal rows older than 7 days are purged by a daily
      cleanup job.

### Non-Functional Requirements

- [x] NFR-SCALE-001: Claiming is safe under multiple app instances
      (row-level lock + `SKIP LOCKED`; no double-claim of one row).
- [x] NFR-SEC-001: OTP plaintext exists only inside `payload` and only
      until a terminal transition or retention purge; it is never
      logged.
- [x] NFR-MAINT-001: No new runtime dependencies and no new
      infrastructure; all tunables live under `app.outbound.*`.
- [x] NFR-PERF-001: Enqueue costs one INSERT inside the existing
...
- [x] Given SMTP is unreachable, when a user registers, then the
      account exists, the HTTP response is 201, and a queue row
      `PENDING` exists (issue #49 AC 1).
- [x] Given a send fails, when each configured delay elapses, then the
      message is re-attempted 3 times at 10s, 30s, and 2m (issue #49
      AC 2).
- [x] Given 3 retries are exhausted, when the 4th attempt fails, then
      the message is persisted as `DEFERRED` for later delivery instead
      of being lost (issue #49 AC 3).
- [x] Given due rows in the queue, when the drainer runs, then pending
      emails are delivered (verified end-to-end with GreenMail)
      (issue #49 AC 4).
- [x] Given sends, failures, and retries have occurred, when reading
      the logs, then counts for all three are visible (issue #49 AC 5).
- [x] Given the whole feature is in place, when the existing test
      suite runs, then all pre-existing email tests still pass
      (issue #49 AC 6).
- [x] Given a concurrent claim scenario, when two drainers claim
